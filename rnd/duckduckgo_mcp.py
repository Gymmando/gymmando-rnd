"""
DuckDuckGo MCP Integration - Complete Version
"""
...
class GymandoDuckDuckGoSearch:
    """Search the web using DuckDuckGo MCP"""
    
    def __init__(self):
        """Initialize the search class"""
        self.client = Anthropic()
        print("✅ GymandoDuckDuckGoSearch initialized!")
    ...
    async def search_with_mcp(self, query: str):
        ...
        # DuckDuckGo doesn't require an API key - it's free to use
        env = {
        ...
        server_params = StdioServerParameters(
            command="npx",
            args=["--quiet", "-y", "duckduckgo-mcp-server@latest"],  # --quiet suppresses npx output, -y auto-installs
            env=env
        )
        ...
        async with stdio_client(server_params) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                print("✅ DuckDuckGo MCP server connected!\n")
                ...
                # Handle tool use - DuckDuckGo might make multiple tool calls
...
if __name__ == "__main__":
    searcher = GymandoDuckDuckGoSearch()
    while True:
