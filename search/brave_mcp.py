"""
Brave Search MCP Integration - Complete Version
"""
...
class GymandoBraveSearch:
    """Search the web using Brave Search MCP"""
    
    def __init__(self):
        """Initialize the search class"""
        self.client = Anthropic()
        print("✅ GymandoBraveSearch initialized!")
    ...
    async def search_with_mcp(self, query: str):
        ...
        env = {
            **os.environ,  # Inherit existing environment
            "BRAVE_API_KEY": brave_api_key,  # Set Brave API key
        ...
        server_params = StdioServerParameters(
            command="npx",
            args=["--quiet", "-y", "@modelcontextprotocol/server-brave-search@latest"],  # --quiet suppresses npx output, -y auto-installs
            env=env
        )
        ...
        async with stdio_client(server_params) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                print("✅ Brave Search MCP server connected!\n")
                ...
                # Handle tool use - Brave might make multiple tool calls
